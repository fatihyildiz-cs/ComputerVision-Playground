...
smile_cascade = cv2.CascadeClassifier('haarcascade_smile.xml') # We load the cascade for the smiles.
...
def detect(gray, frame): # We create a function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        ...
        for (sx, sy, sw, sh) in smiles: # For each detected smile:
            cv2.rectangle(roi_color,(sx, sy),(sx+sw, sy+sh), (0, 0, 255), 2) # We paint a rectangle around the smiles, but inside the referential of the face.    
